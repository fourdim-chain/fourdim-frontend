...
const TokenInfoList = () => {
  ...
  const updateInfosBalance = async (infos: TokenInfoItem[]) => {
    ...
    for (let i = 0; i < infos.length; i++) {
      ...
      const targetAccountBalanceBN = toAddress
        ? await tokenContract.balanceOf(toAddress)
        : ethers.BigNumber.from(0);
      const targetAccountBalance = ethers.utils
        .formatUnits(targetAccountBalanceBN, decimals)
        .toString();
      const curAccountBalanceBN = curAccountAddress
        ? await tokenContract.balanceOf(curAccountAddress)
        : ethers.BigNumber.from(0);
  ...
  const handleContentUpdate = async (infos: TokenInfoItem[]) => {
    ...
    setInfos((old) => {
      const filterDuplicateOldItems = old.filter(
        (item) => !newInfos.find((newItem) => newItem.tokenAddress === item.tokenAddress)
      );
